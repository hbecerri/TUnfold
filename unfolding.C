...
void unfolding(Int_t N = 10000,Int_t bin = 2)
{
      if(gRandom) delete gRandom;
      gRandom = new TRandom3(0);
      gStyle->SetOptStat(0);
    
      TFile* file = TFile::Open("Data_unfolding.root");
...
//           <<" / "<<unfold.GetNdf()<<"\n";
  ...
  //Get input Stat. errors and response matrix statistical errors
...
// Closure Test LCurve_min
    ...
    for(int cc=1;cc<=gen_bin;cc++){
     ...
     TGraphErrors *gr_R = new TGraphErrors(gen_bin,recA,bias_Rt,ex,Final_error_Re);
     C3->SetGrid();
